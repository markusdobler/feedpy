...
class Feedpy(object):
    ...
    def stream_content(self, stream_id, count=25, unread_only=True,
        ...
        return self._get('/streams/contents', params)

    def _post_to_markers(self, action, type, ids, last_entry_id=None):
        data = {
            "action": action,
            "type": type,
            "entryIds": ids,
            "lastReadEntryId": last_entry_id,
        }
        self._post('/markers', data)

    def mark_articles_as_read(self, entry_ids):
        self._post_to_markers('markAsRead', 'entries', entry_ids)

    def mark_articles_as_unread(self, entry_ids):
        self._post_to_markers('keepUnread', 'entries', entry_ids)

    def mark_feed_as_read(self, feed_id, last_read_entry):
        self._post_to_markers('markAsRead', 'feeds', [feed_id], last_read_entry)

    def mark_category_as_read(self, category_id, last_read_entry):
        self._post_to_markers('markAsRead', 'categories', [category_id], last_read_entry)
...
class FeedlyAPI(object):
    ...
    client_id = 'sandbox'
    client_secret = 'FUFNPXDNP2J0BF7RCEUZ'
    ...
    def _get_id_and_tokens(cls, code_or_url):
        ...
        r = requests.post(cls.url('/auth/token'), params=params)
    ...
    def reauthenticate(self):
        ...
        r = self.post('/auth/token', params)
        self._set_authorization(r['access_token'])
    ...
    def global_resource_id(self, kind, name):
        return 'user/%s/%s/global.%s' % (self._user_id, kind, name)
